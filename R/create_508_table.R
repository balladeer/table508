...
create_508_table <- function(.data, table_id = "table",
  ...
  if (rownames_as_headers && is.null(top_left_cell)){
    stop('Can not use rownames_as_headers without setting top_left_cell as well')
  }
  ...
  if (rownames_as_headers){
    xml2::xml_add_child(header_row, "th", top_left_cell, id=sprintf(COL_ID_TEMPLATE, 0))
  }
